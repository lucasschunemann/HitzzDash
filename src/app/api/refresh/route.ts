...
export async function POST() {
  ...
  // Na Vercel a coleta só é enfileirada: quem executa é a rotina semanal (npm run cc -- work).
  ...
  return Response.json({ ...r, message: isWorkerHost() ? `Coleta iniciada para ${r.accounts} contas` : `Coleta de ${r.accounts} contas na fila: roda na próxima atualização (${fmtNextWeeklyUpdate()}) ou ao rodar a rotina manualmente` });
}
