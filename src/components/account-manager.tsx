    ...
    <form onSubmit={submit} className={cn("flex flex-wrap items-center gap-2", compact && "")}>
      <div className="relative min-w-52 flex-1">
        ...
        <input
          ...
          className="h-8.5 w-full rounded-[10px] bg-surface pl-8 pr-3 text-[13px] shadow-sm ring-1 ring-hairline outline-none placeholder:text-ink-3 focus:ring-2 focus:ring-[var(--focus)]"
        />
      </div>
      <select value={group} onChange={(e) => setGroup(e.target.value as keyof typeof GROUPS)} aria-label="Grupo" className="h-8.5 rounded-[10px] bg-surface px-2.5 text-[13px] shadow-sm ring-1 ring-hairline">
...
export function AccountTable({ accounts }: { accounts: AccountStat[] }) {
  ...
  return (
    <Card className="divide-y divide-hairline">
      {accounts.map((a) => (
        <div key={a.id} className={cn("flex flex-wrap items-center gap-x-3 gap-y-2 px-4 py-3", !a.active && "opacity-55")}>
          ...
          <div className="min-w-40 flex-1">
            <div className="flex items-center gap-1.5">
              <a href={`https://www.instagram.com/${a.handle}/`} target="_blank" rel="noreferrer" className="text-[13.5px] font-semibold text-ink hover:underline">
                @{a.handle}
              </a>
          ...
          <select
            ...
            className="h-7.5 rounded-[9px] bg-surface-2 px-2 text-[12.5px] ring-1 ring-hairline"
          >
            {Object.entries(GROUPS).map(([k, v]) => (
              <option key={k} value={k}>
                {v}
