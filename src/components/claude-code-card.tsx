...
function Phrase({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="group inline-flex items-center gap-2 rounded-[9px] bg-surface px-2.5 py-1.5 text-left text-[12.5px] font-medium text-ink shadow-sm ring-1 ring-hairline hover:bg-surface-2"
      title="Copiar"
    >
      “{text}”
      {ok ? <Check className="size-3.5 text-good" /> : <Copy className="size-3.5 text-ink-3 group-hover:text-ink-2" />}
    </button>
  );
}
    ...
    <Card className="p-4 md:p-5">
      <div className="flex flex-wrap items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-[14px] font-semibold text-ink">{idle ? "Claude Code em dia" : "Aguardando o Claude Code"}</div>
          ...
          <div className="mt-2.5 flex flex-wrap items-center gap-2">
            <span className="text-[12px] text-ink-3">No Claude Code, diga:</span>
