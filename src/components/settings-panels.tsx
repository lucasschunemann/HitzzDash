  ...
  return (
    <Card className="divide-y divide-hairline">
      {items.map((i) => {
        ...
        return (
          <div key={i.key} className="flex flex-wrap items-center gap-3 px-5 py-4">
...
const input = "h-8 w-24 rounded-[6px] bg-surface-2 px-2.5 text-right text-[13px] ring-1 ring-hairline outline-none focus:ring-2 focus:ring-[var(--focus)] tabular";
...
export function SettingsForm({ initial, apify }: { initial: S; apify: boolean }) {
  ...
  return (
    <div className="space-y-10">
      ...
      <div>
        <h3 className="mb-3 text-[14px] font-medium text-ink-2">Agendamento</h3>
        <Card className="divide-y divide-hairline">
          <Row label="Coleta automática" hint={apify ? (s.scheduleEnabled ? (next ? `Próxima por volta de ${fmtDateTime(next)}` : "Roda na próxima checagem (a cada minuto)") : "Desligada") : "Precisa de APIFY_TOKEN"}>
            <Switch.Root
              ...
              className="relative h-6 w-10 rounded-full bg-surface-3 transition-colors active:scale-95 data-[state=checked]:bg-accent"
              aria-label="Coleta automática"
            >
              <Switch.Thumb className="block size-5 translate-x-0.5 rounded-full bg-white shadow transition-[transform,background-color] duration-200 ease-[cubic-bezier(.22,1,.36,1)] data-[state=checked]:translate-x-[18px] data-[state=checked]:bg-on-accent" />
            </Switch.Root>
          </Row>
      ...
      <div>
        <h3 className="mb-3 text-[14px] font-medium text-ink-2">Coleta e análise</h3>
        <Card className="divide-y divide-hairline">
          ...
          <Row label="Coletar compartilhamentos" hint="Recurso pago do actor do Apify (includeSharesCount)">
            <Switch.Root checked={s.includeSharesCount} onCheckedChange={(v) => save({ includeSharesCount: v })} className="relative h-6 w-10 rounded-full bg-surface-3 transition-colors active:scale-95 data-[state=checked]:bg-accent" aria-label="Coletar compartilhamentos">
              <Switch.Thumb className="block size-5 translate-x-0.5 rounded-full bg-white shadow transition-[transform,background-color] duration-200 ease-[cubic-bezier(.22,1,.36,1)] data-[state=checked]:translate-x-[18px] data-[state=checked]:bg-on-accent" />
            </Switch.Root>
          </Row>
        </Card>
  ...
  return (
    <Card className="divide-y divide-hairline">
      ...
      {rows.slice(0, 40).map((j) => (
        <div key={j.id} className="flex flex-wrap items-center gap-x-3 gap-y-1 px-5 py-3 text-[13.5px] transition-colors hover:bg-hover">
          {j.status === "running" ? <Loader2 className="size-3.5 animate-spin text-[var(--band-below)]" /> : <span className={cn("size-2 rounded-full", j.status === "done" ? "bg-good" : j.status === "failed" ? "bg-bad" : j.status === "queued" ? "bg-ink-3" : "bg-surface-3")} />}
          <span className="font-medium">
            {j.videoId ? (
              <Link href={`/videos/${encodeURIComponent(j.videoId)}`} className="link-underline">
                {j.label}
              </Link>
            ) : (
              j.label
